	...
	_createClass(ChartPie, [{
		...
		value: function setPie(repeat) {
			...
			this.shapeList.forEach(function (shape) {
				...
				//饼上的名称和值显示格式
				if (typeof _this4.style.nameStyle === 'function') {
					shape.nameText = _this4.style.nameStyle(shape.name, shape.value, shape.precent);
				}
				if (typeof _this4.style.nameStyle === 'string') {
					shape.nameText = _this4.style.nameStyle.replace('{a}', shape.name).replace('{b}', shape.value).replace('{c}', Math.round(shape.precent));
				}
				if (typeof _this4.style.valueStyle === 'function') {
					shape.valueText = _this4.style.valueStyle(shape.name, shape.value, shape.precent);
				}
				if (typeof _this4.style.valueStyle === 'string') {
					shape.valueText = _this4.style.valueStyle.replace('{a}', shape.name).replace('{b}', shape.value).replace('{c}', Math.round(shape.precent));
				}
			});
		...
		value: function foregroundPaint(scene) {
			...
			this.foregroundScene.paint(function (context) {
				...
				//遍历并绘制
				_this5.shapeList.forEach(function (shape) {
					shape.paint(context);
				});

				//绘制图例
				_this5.paintLegend(context);
			});
		}
